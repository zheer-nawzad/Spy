...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // in-progress snapshots once we leave it, no need to keep syncing them
      if (nextPhase === "gallery") merged.liveSnapshots = {};
      setRoom(merged);
  ...
  const netDeadlineRef = useRef(null);
  const netFiredRef = useRef(false);

  // Based on a real clock deadline rather than counting ticks — mobile
  // browsers routinely throttle or pause setInterval in a backgrounded tab
  // (screen off, switched apps, etc). Counting ticks would silently stall
  // or drift in that case; checking "has the deadline passed" self-corrects
  // the instant the tab wakes up, even if several ticks were missed.
  useEffect(() => {
    if (!netTimerActive) {
      netDeadlineRef.current = null;
      netFiredRef.current = false;
      setNetTimeLeft(0);
      return;
    }
    netDeadlineRef.current = Date.now() + room.timerSec * 1000;
    netFiredRef.current = false;
    setNetTimeLeft(room.timerSec);

    const check = () => {
      const dl = netDeadlineRef.current;
      if (!dl) return;
  ...
  // Skipped once the peek has already been used this round — no one can
  // use it again, so there's no reason to keep pushing image data around.
  const netBroadcastActive =
    appMode === "online" && netScreen === "draw" && !!room && room.mode === "classic" &&
    !netMySubmitted && !room.peekUsed;
  useEffect(() => {
    if (!netBroadcastActive) return;
    const id = setInterval(() => {
      const canvas = netCanvasRef.current;
      const cur = roomRef.current;
      if (!canvas || !cur || cur.peekUsed) return;
