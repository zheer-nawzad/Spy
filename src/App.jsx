...
export default function App() {
  ...
  const netBlankCanvasRef = useRef(null); // pristine-fill signature, to detect a wiped canvas
  const netStrokeBackupRef = useRef(null); // last known-good capture, taken after each stroke
  ...
  function netSetupCanvas() {
    ...
    netBlankCanvasRef.current = canvas.toDataURL("image/png");
  ...
  async function netHandlePointerUp(e) {
    ...
    } else if (netCanvasRef.current) {
      // local-only backup, no network cost — a safety net in case the
      // canvas's pixel content gets silently wiped by the browser while
      // the tab is backgrounded (a known mobile quirk), which would
      // otherwise make an auto-submit-on-timeout capture come back blank
      netStrokeBackupRef.current = netCanvasRef.current.toDataURL("image/png");
    }
  }
  ...
  async function netSubmitClassicDrawing() {
    ...
    // If the tab was backgrounded, some mobile browsers silently wipe the
    // canvas's pixel content while the DOM element itself survives — the
    // capture comes back indistinguishable from a freshly-blank board even
    // though real strokes were drawn. Fall back to the last known-good
    // capture taken right after each stroke, rather than submitting blank.
    if (dataURL === netBlankCanvasRef.current && netStrokeBackupRef.current) {
      dataURL = netStrokeBackupRef.current;
    }
