...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const merged = {
        ...base,
        phase: nextPhase,
        // union with what we already have locally (from realtime), not just
        // whichever snapshot is fresher — a submission that arrived via
        // realtime but hadn't yet landed in this fetch must not get silently
        // dropped by overwriting the row with the older snapshot
        readyIds: Array.from(new Set([...(base.readyIds || []), ...(room.readyIds || [])])),
        drawings: { ...(base.drawings || {}), ...(room.drawings || {}) },
        votes: { ...(base.votes || {}), ...(room.votes || {}) },
      };
      if (nextPhase === "draw") {
      ...
      // the peek feature only matters during drawing — drop the accumulated
      // in-progress data once we leave it, no need to keep syncing it
      if (nextPhase === "gallery") {
  ...
  // Nobody broadcasts anything by default. When the Spy picks a target, the
  // room records who's being watched and until when; that ONE player's
  // client then streams frequent snapshots for those few seconds, and the
  // Spy's screen re-renders each new frame as it arrives — a live view, but
  // only exactly when and for whom it's actually needed.
  const netBeingWatched =
    appMode === "online" && netScreen === "draw" && !!room &&
    room.mode === "classic" && room.peekTargetId === myId && !netMySubmitted;
  useEffect(() => {
    if (!netBeingWatched) return;
    let cancelled = false;
    // sync once against the database right as watching starts, so the
    // burst of frequent writes that follows builds on genuinely current
    // data instead of whatever this device happened to have cached
    (async () => {
      const fresh = await loadRoom(roomCode);
      if (!cancelled && fresh) setRoom(fresh);
    })();
    const id = setInterval(() => {
      const cur = roomRef.current;
      const canvas = netCanvasRef.current;
      if (!cur || !canvas) return;
      if (!cur.peekUntil || Date.now() > cur.peekUntil || cur.peekTargetId !== myId) return;
      const dataURL = canvas.toDataURL("image/png");
      const merged = { ...cur, peekFrame: dataURL };
      setRoom(merged);
      netSave(merged);
    }, 400);
  ...
  const [netPeekWatchingName, setNetPeekWatchingName] = useState(null);
  ...
  async function netPeekAt(targetId, targetName) {
    ...
    const cur = roomRef.current;
    if (!cur) return;
    setNetPeekWatchingName(targetName);
    setTimeout(() => {
      setNetPeekWatchingName(null);
      // stop the target's broadcast loop cleanly rather than leaving it idling
      const latest = roomRef.current;
      if (latest && latest.peekTargetId === targetId) {
        const cleared = { ...latest, peekTargetId: null, peekUntil: null, peekFrame: null };
        setRoom(cleared);
        netSave(cleared);
      }
    }, 5000);
    const merged = { ...cur, peekTargetId: targetId, peekUntil: Date.now() + 5000, peekFrame: null, peekUsed: true };
    setRoom(merged);
    netSave(merged);
  }
      ...
      <div className="bz-shell">
        ...
        {netPeekWatchingName && (
          <div className="bz-peek-overlay">
            <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 14 }}>{s.peekWatching(netPeekWatchingName)}</div>
            {room.peekFrame ? (
              <img src={room.peekFrame} alt="peek" />
            ) : (
